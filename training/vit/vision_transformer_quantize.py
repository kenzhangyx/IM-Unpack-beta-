...
def percentile_quantize_fn(inp, percentile, num_dist_vals):
    ...
    vals = inp.abs().reshape(-1)
    if percentile == 1:
        alpha = vals.max()
    else:
        alpha = torch.topk(vals, k = int(vals.numel() * (1 - percentile)), dim = -1).values.min()
    
    # global _stats, _current_name
    # _stats[f"{_current_name}.alpha"].append(alpha.item())
    # _stats[f"{_current_name}.sparsity"].append((inp.abs() < 1e-20).float().mean().item())
...
class PreMMQuantize(torch.autograd.Function):
    ...
    def forward(ctx, A):
        ...
        return percentile_quantize_fn(A, _quantize_config["percentile"], _quantize_config["num_dist_vals"])
...
class PostMMQuantize(torch.autograd.Function):
    ...
    def forward(ctx, A):
        ...
        # ctx._current_name = _current_name
        return A
    ...
    def backward(ctx, grad_A):
        global _quantize_config
        # global _current_name
        # _current_name = f"{ctx._current_name}.backward"
        grad_A = percentile_quantize_fn(grad_A, _quantize_config["backward_percentile"], _quantize_config["backward_num_dist_vals"])
        # _current_name = None
        return grad_A

def quantized_matmul(A, B):
    A = PreMMQuantize.apply(A)
    B = PreMMQuantize.apply(B)
    C = torch.matmul(A, B)
    C = PostMMQuantize.apply(C)
    return C
    
def quantized_linear(x, module, name):
    # global _current_name
    # _current_name = name
    y = quantized_matmul(x, module.weight.T)
    if module.bias is not None:
        y = y + module.bias
    # _current_name = None
    return y

def quantized_attention(x, module, name):
    B, N, C = x.shape
    ...
    # global _current_name
    # _current_name = f"{name}.attn_scores"
    attn = quantized_matmul(q, k.transpose(-2, -1))
    
    attn = attn.softmax(dim=-1)
    attn = module.attn_drop(attn)
    
    # _current_name = f"{name}.attn_out"
    x = quantized_matmul(attn, v)
    # _current_name = None
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        global _quantize_config
        if len(_quantize_config) > 0:
            print(_quantize_config)
            for name, module in self.named_modules():
                if isinstance(module, Attention):
                    print(name, module, "quantized_attention")
                    module.forward = partial(quantized_attention, module = module, name = name)
                    
                if isinstance(module, nn.Linear):
                    print(name, module, "quantized_linear")
                    module.forward = partial(quantized_linear, module = module, name = name)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # global _report_freq
        # _count = _count + 1
        # if _count % _report_freq == 0:
        #     _count = 0
        #     import json
        #     summary = {key:(sum(val)/len(val)) for key, val in _stats.items()}
...
@register_model
def vit_small_patch16_224_qmm_p95n31(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    global _quantize_config
    _quantize_config["percentile"] = 0.95
    _quantize_config["num_dist_vals"] = 31
    _quantize_config["backward_percentile"] = 0.95
    _quantize_config["backward_num_dist_vals"] = 16383
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    print(model)
    print(_quantize_config)
...
@register_model
def vit_small_patch16_224_qmm_p95n31v2(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    global _quantize_config
    _quantize_config["percentile"] = 0.95
    _quantize_config["num_dist_vals"] = 31
    _quantize_config["backward_percentile"] = 0.95
    _quantize_config["backward_num_dist_vals"] = 1023
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    print(model)
    print(_quantize_config)
...
@register_model
def vit_small_patch16_224_qmm_p95n63(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    global _quantize_config
    _quantize_config["percentile"] = 0.95
    _quantize_config["num_dist_vals"] = 63
    _quantize_config["backward_percentile"] = 0.95
    _quantize_config["backward_num_dist_vals"] = 16383
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
...
@register_model
def vit_small_patch16_224_qmm_p95n127(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Small (ViT-S/16)
    """
    global _quantize_config
    _quantize_config["percentile"] = 0.95
    _quantize_config["num_dist_vals"] = 127
    _quantize_config["backward_percentile"] = 0.95
    _quantize_config["backward_num_dist_vals"] = 16383
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('vit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
